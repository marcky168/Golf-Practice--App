...
type LoginFormProps = {
  /** Passed from the server so Vercel runtime env works (not only build-time client bundle) */
  supabaseUrl: string;
  supabaseAnonKey: string;
  siteUrl: string;
};

export function LoginForm({ supabaseUrl, supabaseAnonKey, siteUrl }: LoginFormProps) {
  ...
  const handleGoogleSignIn = async () => {
    if (!supabaseConfigured) {
      toast.error(
        "Supabase is not configured on this deployment. In Vercel → Settings → Environment Variables, add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY (exact names), check Production + Preview, then Redeploy."
      );
      return;
    }
          ...
          <div className="bg-white border border-border rounded-3xl p-7 sm:p-8 shadow-xl shadow-black/5">
            {!supabaseConfigured && (
              <p className="text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2 mb-4">
                Supabase env vars are not visible on the server for this deployment. Fix names in Vercel and redeploy.
              </p>
            )}
