...
### Equipment support is guidance-only — no number capture (reversal)
**Decision:** The app does **not** capture launch monitor or wrist numbers. Switching a device on shows a read-only setup card — which Mevo readouts to put on screen, the window to hold each one in, which HackMotion pattern to select and which tour trace to overlay. Only the good-shot count and which devices were used get saved.
**Why:** The user's words: "I don't need to log numbers on the app but just to show me what I need to pick for my practice." Re-typing a number the Mevo is already displaying costs attention at the range and tells the user nothing the device isn't already telling them. The capture path was solving a problem that didn't exist.
**Deleted:** `BlockTechEntry.tsx` (the per-block entry form, including the iOS +/− sign toggle), `BlockTechData`, `emptyBlockTech`, `blockTechHasData`, `evaluateBlockTech`, `summariseTechChecks`, `pressureSetPassed`, `TechCheck`, `STRIKE_LOCATIONS`, and the `blockTech` / `techOnTargetPct` fields on `ProgramSessionLog`. All of it was written earlier the same session; none of it had shipped.
**Added:** `describeTargets()` and `mevoScreenMetrics()` (pure display formatters), `HackMotionSetup` on `ProgramPhase` (pattern + reference swings + a trace to avoid), `ModuleTargets.tsx`, and richer per-module targets — club path, low point, landing angle, spin windows.
**Consequence:** the "surface equipment data in /insights" follow-up is now moot — there is no equipment data to surface. `TechTargets` is display-only; adding a field means adding a line to `describeTargets`.

**Open / next:**  — `techOnTargetPct` and `blockTech` are persisted in `programLog` but nothing reads them back. Pressure-set enforcement (`pressureSetPassed`) is implemented and unit-verified but not yet wired into a UI gate. Both are natural follow-ups.
