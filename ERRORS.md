...
### Shipped a wrist target that trained the wrong HackMotion release pattern
**Bug:** `wristAtImpact` was set to `{ min: -25, max: 0 }` on both full-swing modules — a heavily bowed lead wrist at impact. I picked it from a general "tour players are bowed at impact" impression without checking what HackMotion actually prescribes.
**Root cause:** HackMotion isn't a hit-this-number device, it's a pattern-matching device built on three Scott Cowx release patterns, and its own app *recommends* Pattern A (Stable Lead Wrist Extension) for the average golfer — "improves clubface control and consistency of ball flight, does not require extreme athleticism or timing." A −25° impact target is Pattern B (the DJ flexion pattern), which carries a permanent timing cost. The app was quietly gating on the pattern the device tells you not to copy.
**Found how:** Only after the user sent screenshots of the HackMotion app's Data tab. Web search alone gave me isolated tour averages (−5 to −10° at impact) with no indication those numbers belong to a specific pattern among several.
**Fix:** Driver `−5..+10`, long iron/FW `−8..+6` (hands further forward on a descending strike), same correction to `DEFAULT_TECH_TARGETS`. Also purged the Pattern B language from the cue cards and drill notes — Module 1's cue said "lead wrist bowing through impact" and Module 2's said "flat to bowed", both of which coach the wrong pattern regardless of what the gate checks.
**Lesson:** A training-aid number is meaningless without the methodology it belongs to. Tour averages are a *distribution across several valid patterns*, not a target — averaging them produces a position no good player actually uses. Before hardcoding any biomechanical target, find out what framework the device teaches and which option within it applies to this user. And when a number will drive someone's physical practice, get the vendor's own prescription rather than inferring one.

### Built a whole data-capture layer the user never wanted
**Issue:** I designed the equipment feature around capturing Mevo and HackMotion numbers — a per-block entry form, evaluation against targets, a scored session summary, persistence. About 400 lines across `equipment.ts`, `BlockTechEntry.tsx` and the session runner. The user's actual need was the opposite direction: "just show me what I need to pick for my practice."
**Root cause:** The v2.0 handoff was written in capture language — "Data the App Should Capture", a 13-field Mevo list, "the app should display or log face-to-path after each shot" — and I implemented it literally. I even asked about *granularity* of capture (per-shot vs per-block) without ever asking whether capture was wanted at all. The question I asked assumed the premise instead of testing it.
**The tell I missed:** I flagged myself that iOS has no Web Bluetooth so every number must be hand-typed, and that per-shot entry would be ~13 taps per ball. That friction was evidence the feature was pointed the wrong way, and I treated it as a UX problem to design around rather than a signal to re-examine the requirement.
**Fix:** Deleted the capture path entirely; replaced it with read-only setup cards driven by pure formatters (`describeTargets`, `mevoScreenMetrics`) plus per-module HackMotion pattern and reference-swing data.
**Lesson:** When a spec describes *storing* something, check whether the user wants it stored or just wants to be *told* it. A launch monitor already has a screen — re-entering what it displays adds work and no information. And when a clarifying question offers options that all share a premise ("how much should we capture?"), the premise itself is the thing that needed asking about.
